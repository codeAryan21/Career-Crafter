...
function ViewApplications() {
    ...
    return applicants ? applicants.length == 0 ? (
        <div className='flex items-center justify-center h-[70vh] '>
            <p className='text-xl sm:text-2xl'>No one has applied for the job yet! Therefore, No applications available</p>
        </div>
    ) : (
                    ...
                    <tbody>
                        {applicants.filter(item => item.jobId && item.userId).map((applicant, index) => (
                            <tr key={index} className='text-gray-700'>
                                ...
                                <td className='px-4 py-2 border-b border-gray-200 max-sm:hidden'>{applicant.jobId.location}</td>
                                <td className='px-4 py-2 border-b border-gray-200'>
                                    <a className='bg-blue-50 text-blue-400 px-3 py-1 rounded inline-flex gap-2 items-center hover:bg-blue-200'
                                ...
                                <td className='px-4 py-2 relative border-b border-gray-200'>
                                    {applicant.status == 'Pending'
                                        ? <div className='relative inline-block text-left group'>
                                            <button className='text-gray-500 action-button'>...</button>
                                            <div className='z-10 hidden absolute right-0 md:left-0 top-0 mt-2 w-32 bg-white border border-gray-200 rounded shadow group-hover:block'>
                                                <button onClick={() => updateJobApplicationStatus(applicant._id, 'Accepted')} className='block w-full text-left px-4 py-2 text-blue-500 hover:bg-gray-100 cursor-pointer'>Accept</button>
                                                <button onClick={() => updateJobApplicationStatus(applicant._id, 'Rejected')} className='block w-full text-left px-4 py-2 text-red-500 hover:bg-gray-100 cursor-pointer'>Reject</button>
                                            </div>
                                        </div>
                                        : <div>
                                            {applicant.status}
